...
void initTest() { //테스트 보드, 피스들을 추가하고 테스트 하세요
    ...
    board[7][2] = (Piece){ 'B', 'W', 0, 0 };
    board[0][7] = (Piece){ 'K', 'B', 0, 0 };
    board[2][6] = (Piece){ 'K', 'W', 0, 0 };
    board[6][0] = (Piece){ 'B', 'W', 0, 0 };
...
void initBoard() {
    ...
    for (int i = 0; i < BOARD_SIZE; i++) {
        board[1][i] = (Piece){ 'P', 'B', 0, 0 }; // 흑색 폰
        board[6][i] = (Piece){ 'P', 'W', 0, 0 }; // 백색 폰
    }
    board[0][0] = board[0][7] = (Piece){ 'R', 'B', 0, 0 }; // 흑색 룩
    board[0][1] = board[0][6] = (Piece){ 'N', 'B', 0, 0 }; // 흑색 나이트
    board[0][2] = board[0][5] = (Piece){ 'B', 'B', 0, 0 }; // 흑색 비숍
    board[0][3] = (Piece){ 'Q', 'B', 0, 0 }; // 흑색 퀸
    board[0][4] = (Piece){ 'K', 'B', 0, 0 }; // 흑색 킹
    board[7][0] = board[7][7] = (Piece){ 'R', 'W', 0, 0 }; // 백색 룩
    board[7][1] = board[7][6] = (Piece){ 'N', 'W', 0, 0 }; // 백색 나이트
    board[7][2] = board[7][5] = (Piece){ 'B', 'W', 0, 0 }; // 백색 비숍
    board[7][3] = (Piece){ 'Q', 'W', 0, 0 }; // 백색 퀸
    board[7][4] = (Piece){ 'K', 'W', 0, 0 }; // 백색 킹
    for (int i = 2; i < 6; i++) {
        for (int j = 0; j < BOARD_SIZE; j++) {
            board[i][j] = (Piece){ ' ', ' ', 0, 0 }; // 미지정
        }
    }
}
// 위의 함수는 체스판 초기화를 담당
...
int point_pv(int s_y, int s_x, int a_y, int a_x) {
    ...
    if (check != 1) {
        if (board[a_y][a_x].type == ' ') {
            board[a_y][a_x].type = '"';
            board[s_y][s_x].moving_point_count += 1;
            return 0;
        }
        else {
...
void along_type(int s_y, int s_x) {
    board[s_y][s_x].moving_point_count = 0;
...
int show_path_of_piece(char select_piece[10]) {
    ...
    if (s_len == 2 && -1 < s_x && s_x < 8 && -1 < s_y && s_y < 8) {
        if (color[gameState.turn % 2] == board[s_y][s_x].color) {
            along_type(s_y, s_x);
            if (board[s_y][s_x].moving_point_count == 0) {
                printf("\n현재 움직일 수 없는 기물입니다.\n\n");
                return 0;
            }
            printBoard();
    ...
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            if(board[i][j].color == color){
                along_type(i, j);
                board_reset();
                if (board[i][j].moving_point_count > 0) {
                    return 1;
                }
            }
        }
